    ...
    async def generate_stream():
        try:
            ...
            if req.step == "research":
                ...
                system_prompt = f"あなたは厳格なファクトチェッカー兼プロの校正者・編集者です。記事本文に「事実誤認や論理の飛躍（校閲）」がないか、および「誤字脱字、文法エラー、表記揺れ、不自然な表現（校正）」がないかを厳密にチェックし、修正箇所をリストアップした上で、修正を反映したリライト後の記事全文を作成してください。{date_context}"
                prompt = f"テーマ: {req.theme}\n\n【最新のWebリサーチ結果】\n{search_context}\n\n【検証対象の記事本文】\n{req.article}\n\n【指示】\n1. 修正が必要な箇所・理由・修正案を、以下の箇条書きフォーマットで出力してください。（表は使わないでください）\n\n【修正箇所1】\n- **該当箇所**: (本文中の該当テキスト)\n- **理由**: (事実誤認、誤字脱字、表記揺れなどの理由)\n- **修正案**: (具体的な修正案)\n\n※修正がない場合は「修正なし」と記載してください。\n2. 指摘リストの下に、「## リライト後の記事本文」という見出しをつけ、上記の修正案を全て反映した完成版の記事全文を出力してください。"
            else:
